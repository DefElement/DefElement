...
from defelement.implementations import implementations, parse_example, sorted_verification_ids
...
if __name__ == "__main__":
    ...
    for e in categoriser.elements:
        ...
        for codename, libname, url, pip in libraries:
            ...
            if len(code_examples) > 0:
                ...
                if codename == "symfem":
                    ...
                    elif e.has_implementation_examples(codename):
        ...
        if len(refs) > 0:
            ...
            for rindex, r in enumerate(refs):
                ...
                content += (
                    f" [<a href='/elements/bibtex/{e.filename}-{rindex}.bib'>BibTeX</a>]</li>\n"
                )
                with open(
        ...
        if e.created is not None:
            ...
            content += "<table class='element-info'>"
            content += (
                f"<tr><td>Element&nbsp;added</td><td>{e.created.strftime('%d %B %Y')}</td></tr>"
            )
    ...
    for i in sorted_verification_ids:
    ...
    content += (
        "<label><input type='checkbox' checked id='check-ref-all' onchange='do_filter_refall()'"
    )
    ...
    content += (
        "<label><input type='checkbox' checked id='check-cat-all'onchange='do_filter_catall()'"
    )
    content += ">&nbsp;show all</label> "
    for c in categoriser.categories:
    ...
    content += (
        f"<h2>Recently updated elements <a href='/updated-elements.xml'>{rss_icon}</a></h2>\n"
    )
    content += "<ul>\n"
